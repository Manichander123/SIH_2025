...
import { X, MapPin, Calendar, Clock, Star, Camera, Thermometer, Users, AlertCircle } from 'lucide-react';
...
const DestinationDetail: React.FC<DestinationDetailProps> = ({ isOpen, onClose, destination }) => {
  ...
  const detailedInfo = {
    ...
    weather: getWeatherInfo(destination.name)
  };

  function getHistoryInfo(name: string): string {
    ...
    return historyMap[name] || 'Rich in history and cultural significance, this destination offers visitors a glimpse into India\'s diverse heritage.';
  }

  function getThingsToDo(name: string): string[] {
  ...
  function getNearbyAttractions(name: string): string[] {
    ...
    return nearbyMap[name] || ['Local temples and monuments', 'Traditional markets', 'Scenic viewpoints', 'Cultural centers'];
  }

  function getTravelTips(name: string): string[] {
  ...
  function getWeatherInfo(name: string): { season: string; temp: string; description: string }[] {
    return [
